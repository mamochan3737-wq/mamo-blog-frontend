...
export async function generateMetadata({ params }: { params: { category: string } }) {
  const category = await client.fetch<Category>(categoryQuery, { category: params.category });
  if (!category) {
...
export default async function CategoryPage({ params }: { params: { category: string } }) {
  const [posts, category] = await Promise.all([
    client.fetch<Post[]>(postsQuery, { category: params.category }),
    client.fetch<Category>(categoryQuery, { category: params.category })
  ]);
